...
get_folder_config <- function(data_root = Sys.getenv("PAPER_SEARCH_DATA_ROOT", 
  ...
  list(
    ...
    rds_persons_folder = Sys.getenv("PAPER_SEARCH_RDS_PERSONS",
                                    unset = file.path(data_root, "rds_persons")),
    ...
    pqt_patch_folder = Sys.getenv("PAPER_SEARCH_PQT_PATCH",
...
ensure_folders <- function(config = NULL) {
  ...
  folders <- c(
    ...
    config$repec_folder,
    config$rds_folder,
    config$rds_persons_folder,
    config$pqt_folder,
    config$pqt_diff_folder,
    config$pqt_patch_folder,
    config$db_folder,
    config$logs
  )
